...
class BaseSprite():
    ...
    def set_movement_direction(self, new_movement_direction: pygame.Vector2):
        if new_movement_direction.magnitude() == 0:
            ...
            self.velocity_vector = new_movement_direction.normalize()*self.velocity_vector.magnitude()
    ...
    def move_object_relative_to_camera(self, camera_movement_vector: pygame.Vector2, vec_in_screen_coord_frame: bool = True):
        if vec_in_screen_coord_frame: # vector provided is in screen coordinate frame
            camera_movement_vector_3d = project_screen_to_3d(camera_movement_vector, self.z)
            camera_movement_vector = pygame.Vector2(camera_movement_vector_3d.x, camera_movement_vector_3d.y)
        else:
...
if __name__ == "__main__":
    SPRITES = [BaseSprite(width=10, height=20, x=50, y=50, velocity_vector=pygame.Vector2(1,0), colour=pygame.Color(255,0,0))]
